<template>

<div>

    <div class=" bg-no-repeat bg-cover overflow-hidden w-[400px] m-8 h-[250px] mt-8 shadow-lg rounded-lg p-8" style="background-image:url('data:image/png;base64,');">

        <h1 class="text-2xl pb-5 text-slate-800 font-bold">
            <slot name="Title"  > 
            </slot>
        </h1>


        <div class="flex flex-row justify-between">
            <span class="text-xl font-medium  text-slate-700 "><i class="fa-solid fa-user-tag"></i></span>
            <div v-for="tags in post.tags" :key="tags">
                <h1 class="text-xl font-medium  text-slate-700 pb-5">
                    {{tags}}
                </h1>
            </div>
        </div>
        
        <div>
            <p class="font-xl font-medium text-slate-800">June 7 ,2022</p>
        </div>

        <div class="flex flex-row  justify-center items-center">
            <router-link :to="'/posts/' + post.id " class="flex flex-col justify-center items-center pl-10 pt-2 hidden group-hover:block  bg-gray-500 text-white h-10 w-40 rounded-lg font-bold">Read more</router-link>
        </div>
    </div>


</div>


</template>

<script>

export default {
    name: 'postCard',
 data: () => ({
        post:
        
            []
        
    }),
    
    // async mounted() {

    //     let result = await axios.get("/posts?limit=150")
    //     console.log('api data', result.data.posts);
    // },
    components: {
        }
    // props: {
    //     post: {
    //          required: true,
    //        type: Object
    //     },

    // }
}
</script>
