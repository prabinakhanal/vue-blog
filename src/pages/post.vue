<template>
  <navBar />
  <div style="background-image: url('')" class="container mx-auto py-10">
    <div class="grid gap-5 grid-cols-3">
      <post-card v-for="(post, index) in trimedData" :key="index" :post="post">
        ...
        <template v-slot:tags>
          <div class="mt-3 flex flex-wrap gap-3">
            <div
              ...
              :key="tag"
              class="border border-gray-100 rounded-full text-sm px-3 py-1 italic"
            >
              # {{ tag }}
            </div>
          </div>
    ...
    <div class="text-gray-300 flex items-center p-10 space-x-8 justify-center">
      <div
        class="text-gray-300 flex items-center p-10 space-x-8 justify-center"
      >
        <div v-for="(pagenumbers, index) in count" :key="index">
          <button
            v-on:click="buttons(index)"
            class="bg-gray-500 text-white h-10 w-20 hover:bg-blue-500 rounded-lg m-3 font-bold"
          >
            {{ pagenumbers }}
          </button>
        </div>
